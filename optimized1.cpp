...
__m256d multiply_and_add(__m256d a, __m256d b, __m256d c);
...
double getMatrixValue(double *vecX, double *vecY, int length_size) {
	...
	for (int i = 0; i < length_size; i+=4) {
		__m256d a = _mm256_set_pd(vecX[i], vecX[i+1], vecX[i+2], vecX[i+3]);
		__m256d b = _mm256_set_pd(vecY[i], vecY[i + 1], vecY[i + 2], vecY[i + 3]);
